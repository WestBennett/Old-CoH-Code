...
namespace CoH_Forums_Parser
{
    class Program
    {
        ...
        const string ConnectionString = "Server=192.185.6.213; Database=wbennett_OldCoHForums;Uid=wbenn_user;Pwd=seGPJ6d74Jlx8OuB;default command timeout=120;"; //MySql Connection string
        ...
        static void Main(string[] args)
        {
            ...
            do
            {
                try
                {
                    ...
                    //Delete all already processed files, and re-get the list
                    ...
                    foreach (string fileToCheck in filesToProcess)
                    {
                        if (filesProcessed.Contains(Path.GetFileNameWithoutExtension(fileToCheck)))
                        {
                            ...
                            Console.WriteLine("Deleted already processed file '" + fileToCheck + "'");
                        }
                        else
                        {
                            //Do nothing
